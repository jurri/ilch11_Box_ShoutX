...
echo '<script type="text/javascript">
  function del() {
    if (anz = prompt("Wieviele Einträge sollen erhalten bleiben?\n(Es werden die zuletzt geschriebenen erhalten)", "0")) {
      if (anz >= 0) { window.location.href = "index.php?gfa_shoutbox-delall-"+anz; }
...
while ($row = db_fetch_assoc($erg) ) {
 $class = ( $class == 'Cmite' ? 'Cnorm' : 'Cmite' );
  ...
  if ( is_siteadmin() ) {
	  echo '<a href="index.php?gfa_shoutbox-d'.$row['id'].'"><img src="include/images/icons/del.gif" alt="'.$lang['delete'].'" title="'.$lang['delete'].'"></a>&nbsp;';
	}
...
echo "<center>&copy;<a href=\"http://www.graphics-for-all.de\" target=\"_blank\"> Graphics for All</a></center>";
$design->footer();
?>